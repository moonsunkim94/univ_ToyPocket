...
// CCompanySearchDlg 대화 상자입니다.

IMPLEMENT_DYNAMIC(CCompanySearchDlg, CDialogEx)

CCompanySearchDlg::CCompanySearchDlg(CWnd* pParent /*=NULL*/)
	: CDialogEx(CCompanySearchDlg::IDD, pParent)
	, m_csearch(_T(""))
{

}

CCompanySearchDlg::~CCompanySearchDlg()
{
}

void CCompanySearchDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialogEx::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_COMBO_SEARCH, m_ccombobox);
	DDX_Text(pDX, IDC_EDIT7, m_csearch);
	DDX_Control(pDX, IDC_LIST1, m_cs_List);
	DDX_Control(pDX, IDOK, m_combtn);
}


BEGIN_MESSAGE_MAP(CCompanySearchDlg, CDialogEx)
	ON_BN_CLICKED(IDOK, &CCompanySearchDlg::OnBnClickedOk)
	ON_NOTIFY(HDN_ITEMCLICK, 0, &CCompanySearchDlg::OnHdnItemclickList1)
	ON_NOTIFY(NM_DBLCLK, IDC_LIST1, &CCompanySearchDlg::OnNMDblclkList1)
	ON_WM_QUERYDRAGICON()
	ON_WM_SYSCOMMAND()
...
// CCompanySearchDlg 메시지 처리기입니다.

void CCompanySearchDlg::OnCustomdrawList(NMHDR* pNMHDR, LRESULT* pResult)
{
...
void CCompanySearchDlg::OnBnClickedOk()
{
	...
	try
	{
		...
		if(nIndex == 0){
			strQuery.Format(_T("SELECT * FROM Company WHERE c_code LIKE '%%%s%%' OR c_name LIKE '%%%s%%' OR c_sort LIKE '%%%s%%' OR c_type LIKE '%%%s%%' OR c_business LIKE '%%%s%%' OR c_ceoname LIKE '%%%s%%' OR c_tel LIKE '%%%s%%'"),
				m_csearch,m_csearch,m_csearch,m_csearch,m_csearch,m_csearch,m_csearch);
		}
		if(nIndex == 1){
			strQuery.Format(_T("SELECT * FROM Company WHERE c_code LIKE '%%%s%%'"),m_csearch);
		}
		if(nIndex == 2){
			strQuery.Format(_T("SELECT * FROM Company WHERE c_name LIKE '%%%s%%'"),m_csearch);
		}
		if(nIndex == 3){
			strQuery.Format(_T("SELECT * FROM Company WHERE c_type LIKE '%%%s%%'"),m_csearch);
		}
		if(nIndex == 4){
			strQuery.Format(_T("SELECT * FROM Company WHERE c_business LIKE '%%%s%%'"),m_csearch);
		}
...
void CCompanySearchDlg::OnHdnItemclickList1(NMHDR *pNMHDR, LRESULT *pResult)
{
...
void CCompanySearchDlg::OnNMDblclkList1(NMHDR *pNMHDR, LRESULT *pResult)
{
...
BOOL CCompanySearchDlg::OnInitDialog()
{
	...
	m_bAscending = FALSE;
	m_cs_List.SortItems(&CCompanySearchDlg::CompareItem, (DWORD_PTR)this);
	UpdateArrow();
	...
	try
	{
		...
		CString strQuery;
		strQuery.Format(_T("SELECT com_id, c_code, c_name,  c_type, c_business, c_ceoname, c_tel, c_number, c_address, c_mobile, c_email, c_birth, c_memo  FROM Company WHERE c_sort = '납입처'"));
...
int CALLBACK CCompanySearchDlg::CompareItem(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
{
...
void CCompanySearchDlg::UpdateArrow(void)
{
...
HCURSOR CCompanySearchDlg::OnQueryDragIcon()
{
...
void CCompanySearchDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
	// TODO: 여기에 메시지 처리기 코드를 추가 및/또는 기본값을 호출합니다.
...
HBRUSH CCompanySearchDlg::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
